...
for (let i = allTwists.length - 1; i > 0; i--) {
    // Genera un indice casuale tra 0 e i
    const j = Math.floor(Math.random() * (i + 1));

    // Scambia gli elementi twists[i] e twists[j]
    [allTwists[i], allTwists[j]] = [allTwists[j], allTwists[i]];
}

function loadTwists(limit, offset=0){
    return allTwists.slice(offset, offset+limit);
}
